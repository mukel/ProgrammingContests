...
void build( int x, int lo, int hi ) {
	if ( lo == hi ) {
		T[x].sum = T[x].L = T[x].C = T[x].R = val[lo];
		return ;
	}
...
int my_query( int loi, int hii, int loj, int hij ) {

	// no overlap
	if ( hii < loj ) {
		query( 0, 1, N, loi, hii ); int r = R;
		query( 0, 1, N, loj, hij ); int l = L;
		return sum[loj - 1] - sum[hii] + l + r;
	}

	query( 0, 1, N, loj, hii );
	int ans = C;
	
	if ( hii < hij ) ans = max( ans, my_query( loi, hii, hii+1, hij ) );
	if ( loi < loj ) ans = max( ans, my_query( loi, loj-1, loj, hij ) );
...
int main() {
	...
	while ( TC-- ) {
		...
		while ( Q-- ) {
			int loi, hii, loj, hij;
			scanf( "%d %d %d %d", &loi, &hii, &loj, &hij );
			printf( "%d\n", my_query( loi, hii, loj, hij ) );
		}
	}
